    ...
    always_ff @(posedge clk) begin
        for (i = 0; i < {len(terms)}; i = i + 1)
            ...
            for layer_i in range(1, reduce_depth_count+1):
                ...
                statements.append(f"logic [{2+layer_i}:0] {buf_prefix}l{layer_i} [0:{next_term_count-1}];")
                statements.append(f"reg [{2+layer_i}:0] {buf_prefix}l{layer_i}_out [0:{next_term_count-1}];")
                if term_count >= 2:
    ...
    always_ff @(posedge clk) begin
        for (i = 0; i < {next_term_count}; i = i + 1)
            {buf(layer_i, "i")} <= {buf(layer_i, "i", postfix="_out")};
                    ...
                    statements.append(f"assign {buf(layer_i, i)} = {buf(layer_i-1, i)};")
        ...
        for i, value in enumerate(discrete_values):
            if value in leaf_ids_by_value:
                groups = combine_leaf_groups(leaf_ids_by_value[value])
                statements.append("")
                statements.append(f"// {value}")
                statements.extend(generate_leaf_prefix_summer(i, groups))
            else:
                statements.append(f"assign val[{i}] = {0};")
